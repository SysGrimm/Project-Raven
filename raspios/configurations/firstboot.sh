...
configure_kodi_settings() {
    echo "[CONFIG]  Configuring Kodi settings..."
...
# Function to apply video optimizations
apply_video_optimizations() {
    echo "[VIDEO] Applying LibreELEC-style video optimizations..."
    
    if [ -f "/opt/raven/optimize-video.sh" ]; then
        /opt/raven/optimize-video.sh
    else
        echo "[WARNING]  Video optimization script not found, skipping..."
    fi
    
    echo "[SUCCESS] Video optimizations applied"
}

# Function to setup boot splash (MANDATORY)
setup_boot_splash() {
    echo "[THEME] Setting up Project Raven boot splash screen..."
    
    # Ensure logo exists
    if [ ! -f "/opt/raven/configurations/logo.png" ]; then
        echo "[ERROR] CRITICAL: Project Raven logo not found at /opt/raven/configurations/logo.png"
        echo "[ERROR] Boot splash is mandatory for Project Raven systems"
        exit 1
    fi
    
    # Try Plymouth splash first (more advanced)
    if [ -f "/opt/raven/configure-boot-splash.sh" ]; then
        echo "[IMAGE] Installing Plymouth boot splash with logo..."
        if /opt/raven/configure-boot-splash.sh; then
            echo "[SUCCESS] Plymouth boot splash configured successfully"
        else
            echo "[WARNING]  Plymouth setup failed, trying simple splash..."
            if [ -f "/opt/raven/simple-boot-splash.sh" ]; then
                if /opt/raven/simple-boot-splash.sh; then
                    echo "[SUCCESS] Simple boot splash configured as fallback"
                else
                    echo "[ERROR] CRITICAL: Both Plymouth and simple splash failed"
                    echo "[ERROR] Boot splash is mandatory for Project Raven systems"
                    exit 1
                fi
            else
                echo "[ERROR] CRITICAL: No fallback boot splash available"
                echo "[ERROR] Boot splash is mandatory for Project Raven systems"
                exit 1
            fi
        fi
    elif [ -f "/opt/raven/simple-boot-splash.sh" ]; then
        echo "[MEDIA] Installing simple boot splash..."
        if /opt/raven/simple-boot-splash.sh; then
            echo "[SUCCESS] Simple boot splash configured successfully"
        else
            echo "[ERROR] CRITICAL: Simple boot splash failed"
            echo "[ERROR] Boot splash is mandatory for Project Raven systems"
            exit 1
        fi
    else
        echo "[ERROR] CRITICAL: No boot splash scripts found"
        echo "[ERROR] Boot splash is mandatory for Project Raven systems"
        exit 1
    fi
    
    echo "[SUCCESS] Boot splash setup completed (MANDATORY)"
}
