...
echo "[LAUNCH] LibreELEC Image Customization"
echo "================================="
...
get_latest_release() {
    ...
    if [ -z "$tag_name" ]; then
        echo "[ERROR] Could not extract version tag from API response"
        echo "Raw API response: $release_info"
        exit 1
    fi
    ...
    if curl --connect-timeout 10 --max-time 30 --output /dev/null --silent --head --fail "$download_url" 2>/dev/null; then
        echo "[SUCCESS] Download URL verified"
    else
        echo "[WARNING]  Download URL verification failed, but continuing..."
        echo "[INFO]  URL: $download_url"
        echo "[INFO]  This might work during actual download (some servers block HEAD requests)"
    fi
...
customize_image() {
    ...
    echo "[THEME] Customizing LibreELEC image..."
    ...
    echo "[CONFIG] Mounting image partitions..."
    ...
    if [ -f "$CONFIG_DIR/config.txt" ]; then
        echo "[SETUP] Applying boot config.txt..."
        sudo cp "$CONFIG_DIR/config.txt" boot/
    fi
    
    if [ -f "$CONFIG_DIR/cmdline.txt" ]; then
        echo "[SETUP] Applying boot cmdline.txt..."
    ...
    if [ -d "$CONFIG_DIR/storage" ]; then
        echo "[FOLDER] Adding custom storage files..."
        ...
        if sudo mountpoint -q storage 2>/dev/null; then
            ...
            echo "[INFO]  Storage partition not available - files will be added via first-boot script"
        fi
    fi
    
    # Add first-boot customization script
    echo "[LAUNCH] Adding first-boot customization script..."
    ...
    echo "[SUCCESS] Customization complete: $custom_image"
    echo "$custom_image"
}
...
main() {
    ...
    if ! source_image=$(download_libreelec "$download_url"); then
        echo "[ERROR] Failed to download LibreELEC image"
        exit 1
    fi
    
    echo "[THEME] Customizing image..."
    if ! custom_image=$(customize_image "$source_image"); then
        echo "[ERROR] Failed to customize image"
        exit 1
    fi
